...
def get_parameter():
    ...
    parser.add_argument('--model_path', type=str, default="saved_dict/",
    ...
    parser.add_argument('--bert_config_path', type=str,
                        default="pretrained/bert-base-uncased/",
                        help='pretrained bert config path')
...
def load_one_data(args, file_name, vocab, tokenizer, block_shuffle=True, is_shuffle=True, is_print=False):
    if is_print:
        print('Loading data from {} with batch size {}...'.format(file_name, args.batch_size))
    one_dataset = ABSA_Dataset(args, file_name, vocab, tokenizer)  # get dataset
...
def load_data(args, vocab, tokenizer=None):
    train_dataloader = load_one_data(args, file_name=args.data_dir + '/train_con_new.json',
                                     vocab=vocab, tokenizer=tokenizer, is_shuffle=True, is_print=True)

    valid_dataloader = load_one_data(args, file_name=args.data_dir + '/valid_con_new.json',
                                     vocab=vocab, tokenizer=tokenizer, is_shuffle=False, is_print=True)
