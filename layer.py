...
class H_TransformerEncoder(nn.Module):
    ...
    def __init__(self, d_model=512,
        ...
        self.nhead = nhead
        self._reset_parameters()  # initiate parameters
...
class H_TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask, src_key_padding_mask, isindi=True):
        ...
        for idx, layer in enumerate(self.layers):
            # 多头mask attn_mask=构造一个自注意力机制中使用的掩码张量
            ...
            attn_mask = torch.stack([attn_mask for _ in range(self.nhead)], dim=1).contiguous().view(-1, L,
                                                                                                     L)  # copy nhead
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        ...
        src_add_norm = self.norm(src + self.dropout(src2))  # 残差连接
