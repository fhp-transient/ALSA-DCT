...
class ALSA_DCT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # aspect-level
        Intra_context_input = (
            length[map_AS], bert_length[map_AS], word_mapback[map_AS], aspect_indi, bert_sequence, bert_segments_ids,
            adj[map_AS], con_spans)

        Intra_context_output = self.intra_context_module(Intra_context_input)

        # sentence-level
        if self.args.plus_AA and map_AA.numel():
...
class Intra_context(nn.Module):
    def __init__(self, args):
        ...
        bert_config = BertConfig.from_pretrained(args.bert_config_path)
        bert_config.output_hidden_states = True
        bert_config.num_labels = 3

        self.layer_drop = nn.Dropout(args.layer_dropout)
        self.context_encoder = BertModel.from_pretrained(args.bert_model_path, config=bert_config)
    ...
    def forward(self, inputs):
        ...
        bert_out = self.layer_drop(bert_out)  # 8*35*768

        # rm [CLS] 得到每个token的特征值
        bert_seq_indi = ~sequence_mask(bert_lengths).unsqueeze(dim=-1)  # get mask
        bert_out = bert_out[:, 1:max(bert_lengths) + 1, :] * bert_seq_indi.float()  # bert_out * mask  8*29*768
        word_mapback_one_hot = (F.one_hot(word_mapback).float() * bert_seq_indi.float()).transpose(1, 2)  # 8*24*29
        bert_out = torch.bmm(word_mapback_one_hot.float(), self.dense(bert_out))  # 8*24*29 and 8*29*200

        # average bert_out/token_num
        wnt = word_mapback_one_hot.sum(dim=-1)  # 8*24 一个单词可能有多个token
        wnt.masked_fill_(wnt == 0, 1)  # 防止除以0
        ...
        span_matrix = get_span_matrix_4D(con_spans.transpose(0, 1))  # get adj matrix for con
        ...
        if self.args.con_dep_version == 'con_add_dep':
            ...
            # adj * span
            adj_matrix = adj.unsqueeze(dim=0).repeat(N, 1, 1, 1)  # 1*8*24*24 -> 3*8*24*24
            assert ((adj_matrix[0] != adj_matrix[1]).sum() == 0)  # assert copy
            span_matrix = (span_matrix * adj_matrix).bool()
        # 8*24*200
...
class Inter_context(nn.Module):
    def __init__(self, args, sent_encoder=None):
        ...
        if not args.borrow_encoder:
            self.sent_encoder = BertModel.from_pretrained(args.bert_model_path)
        self.dense = nn.Linear(args.bert_hidden_dim, args.hidden_dim)
    ...
    def forward(self, as_features, inputs, context_encoder=None):
        ...
        rela_v_inner = torch.cat((inner_v_node.sum(dim=1), inner_v), dim=-1)  # 4*968
    ...
    def forward_bert_inner(self, aa_choice_inner_bert, aa_choice_inner_bert_length, context_encoder=None):
        ...
        bert_outputs = self.sent_encoder(aa_choice_inner_bert) if context_encoder is None else context_encoder(
            aa_choice_inner_bert)
        bert_out, bert_pool_output = bert_outputs.last_hidden_state, bert_outputs.pooler_output
        bert_out = self.layer_drop(bert_out)  # 4*17*768
        ...
        inner_v_node = self.dense(bert_out)  # 4*15*200
    ...
    def con_aspect_graph(self,
        ...
        B = map_AS.max() + 1
        L = map_AA_idx.max() + 1
        graph_input_features = torch.zeros((B, L, as_features.shape[-1]), device=as_features.device)  # 4*3*968
        # 4*3*968
