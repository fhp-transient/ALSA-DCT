...
class ABSA_Dataset(Dataset):
    ...
    def process(self, data, vocab, args, tokenizer):
        ...
        for d in data:
            ...
            text_raw_bert_indices = text_raw_bert_indices[:max_len]  # 取max_len
            ...
            con_path_dict, con_children = get_path_and_children_dict(con_head)  # 获取每个节点到root的路径
            ...
            if args.con_dep_conditional:
                father = 1
                if father in con_children and [con_mapnode[node] for node in con_children[father]].count('S[N]') > 1 \
                        and con_mapnode[father] == 'S[N]':
                    cd_span = spans[node2layerid[father] + 1]
                    cd_adj = get_con_adj(father, length, cd_span, con_children, con_mapnode)
            ...
            for aspect in d['aspects']:
                ...
                select_spans, span_indications = form_aspect_related_spans(con_lca, spans, con_mapnode, node2layerid,
                                                                           con_path_dict)

                select_spans = select_func(select_spans, args.max_num_spans, length)  # spans多退少补为max_num_spans
            ...
            for aa_info in d['aa_choice']:
                ...
                if select_ in choice_list:  # choicen
                    ...
                    select_words = d['token'][word_range[0]:word_range[-1] + 1] if (
                            word_range[0] <= word_range[-1]) else ['and']
                    #  PS函数
...
def ABSA_collate_fn(batch):  # how to get batch
    ...
    for idx in range(batch_size):  # padding the dep matrix
    ...
    bert_sequence = get_long_tensor(bert_sequence, as_batch_size)  # padding
    ...
    if aa_batch_size > 0:
        ...
        map_AA_idx = torch.LongTensor(
            [m + len(a_i) + 1 for a_i in aa_choice_inner_bert_id_list_ for m in range(len(a_i))])
...
class ABSA_DataLoader(DataLoader):
    ...
    def __iter__(self):
        if self.is_shuffle:
            self.dataset.data = self.block_shuffle(self.dataset.data, self.batch_size, self.sort_bs_num, self.sort_key,
                                                   self.is_shuffle)
...
def get_float_tensor(tokens_list, batch_size):
    """ Convert list of tokens to a padded FloatTensor. """
    token_len = max(len(x) for x in tokens_list)
